...
Lo sleep è configurato interamente in minuti per semplificare test e gestione da Home Assistant.
    ...
    - action: set_sleep_minutes
      ...
      then:
        - number.set:
            ...
            value: !lambda |-
              if (minutes < 15.0f) return 15.0f;
              if (minutes > 1440.0f) return 1440.0f;
              return minutes;
  ...
  - platform: template
    ...
    set_action:
      - lambda: |-
          ...
          ESP_LOGI("tankmonitor", "Sleep interval set to %.0f min", x);
    ...
    then:
      ...
      - deep_sleep.enter:
          ...
          sleep_duration: !lambda |-
            ...
            const float minutes = id(sleep_minutes_value);
            return static_cast<uint32_t>(
              minutes * 60.0f * 1000.0f
            );
```
...
```text
15:00 utente imposta sleep = 360 min (6 h)
15:00 XIAO dorme

18:00 XIAO si sveglia
18:00 HACS rileva il collegamento
18:00 HACS chiama set_sleep_minutes(360)
18:00 XIAO salva 360 min
18:00 HACS marca configurazione sincronizzata
18:00 XIAO dorme 360 min (6 h)
```
